...
export default function BarcodeScanner({ onResult, onClose }: Props) {
  ...
  const lookup = useCallback(async (barcode: string) => {
    ...
    try {
      ...
      else     setState({ status: 'not_found', barcode: code })
    } catch {
  ...
  const rescan = () => setState({ status: 'scanning' })
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className={`relative rounded-2xl overflow-hidden bg-black ${state.status !== 'scanning' ? 'hidden' : ''}`}>
        ...
        {torchAvail && (
          <button
            type="button"
            onClick={toggleTorch}
            className={`absolute top-3 right-3 w-9 h-9 rounded-full flex items-center justify-center backdrop-blur transition ${torchOn ? 'bg-yellow-400 text-gray-900' : 'bg-black/50 text-white'}`}
            aria-label="Lampe torche"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 2h12v3a2 2 0 0 1-.6 1.4L15 9v11a2 2 0 0 1-2 2h-2a2 2 0 0 1-2-2V9L6.6 6.4A2 2 0 0 1 6 5z"/></svg>
          </button>
        )}
      ...
      {state.status === 'found' && (
        <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-3">
            <Button variant="secondary" className="flex-1 justify-center" onClick={rescan}>
              Re-scanner
            </Button>
      ...
      {state.status === 'error' && (
        <div className="flex flex-col gap-3 items-center py-4">
          ...
          <Button variant="secondary" onClick={rescan}>Reessayer</Button>
        </div>
      )}
      ...
      {state.status === 'scanning' && (
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            ...
            />
            <Button size="sm" onClick={() => lookup(manual)} disabled={!manual.trim()}>OK</Button>
          </div>
        </div>
      )}
